...
const SHEET_KEY = {
  ...
  /* ── 5. 빛과 전자기 (2026-09-21 사용자 붙여넣기로 확정) ──────────────────────
     5-3·5-4는 시트와 로컬 표기가 같아 등록이 필요 없다(기본값 `5-3`/`5-4` + 문자 접두로 통한다).
     5-5만 중단원·아티클이 **둘 다** 다르다 — 게다가 문자 접두가 없어 기본값은 아티클명을
     통째로(`측광과 분광`) 보내므로 반드시 어긋난다. 값의 출처는 사용자가 붙여넣어 준 5단원 탭 행이다. */
  '5|빛으로 읽는 우주':
      { 중단원: '5-5) 빛으로 읽는 우주', 아티클: '빛으로 읽는 우주' },
      /* 로컬 중단원만 다르다 — `5-5) 빛으로 우주를 읽다`.
         2026-09-21 사용자 지시로 아티클명을 `측광과 분광` → `빛으로 읽는 우주`로 바꿨다
         (1단원 `1-4) B. 측광과 분광`과 이름이 겹쳐 혼동됐다). JSON·스켈레톤·파일명 모두 변경. */
